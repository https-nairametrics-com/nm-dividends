...
class Resource(models.Model):
    ...
    tags = models.JSONField(
        default=list,
        blank=True,
        help_text="List of tags (maximum 10)"
    )
    ...
    featured_media = models.CharField(
        max_length=500,
        blank=True,
        help_text="URL to featured image or media"
    )
    ...
    featured = models.BooleanField(
        default=False,
        help_text="Whether this resource is featured"
    )
...
class MediaFile(models.Model):
    ...
    original_filename = models.CharField(
        max_length=255,
        help_text="Original filename before renaming"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.file_type and self.original_filename:
            ext = self.original_filename.split('.')[-1].lower() if '.' in self.original_filename else ''
            self.file_type = ext
